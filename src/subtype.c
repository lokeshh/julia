...
static jl_value_t *union_oracle(jl_value_t *u, jl_stenv_t *e, int8_t R)
{
...
{
    jl_value_t *choice = union_oracle((jl_value_t*)u, e, R);
...
{
    if (x == jl_ANY_flag)
        x = (jl_value_t*)jl_any_type;
    if (y == jl_ANY_flag)
        y = (jl_value_t*)jl_any_type;
    if (jl_is_uniontype(x)) {
        if (x == y) return 1;
        x = union_oracle(x, e, 0);
    }
    if (jl_is_uniontype(y)) {
        ...
        if (jl_is_typevar(x)) {
            ...
            // if matching `typevar <: union`,
            // first try to match the whole union against the variable
            // before trying to take it apart (to see if there are any variables lurking inside)
