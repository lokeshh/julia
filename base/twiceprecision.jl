...
function (::Type{TwicePrecision{T}}){T,I}(nd::Tuple{I,I}, nb::Integer)
    step_n, step_d = nd
    x = TwicePrecision{T}(step_n, 0) / step_d
    step_hi = truncbits(x.hi, nb)
    TwicePrecision{T}(step_hi, x.lo + (x.hi - step_hi))
end
...
convert{T}(::Type{T}, x::TwicePrecision{T}) = convert(T, x.hi + x.lo)
...
function floatrange{T}(::Type{T}, start_n::Integer, step_n::Integer, len::Integer, den::Integer)
    step_n == 0 && throw(ArgumentError("range step cannot be zero"))
    if len < 2
...
function floatrange(a::AbstractFloat, st::AbstractFloat, len::Real, divisor::AbstractFloat)
    ...
    StepRangeLen(T(a/divisor), T(st/divisor), Int(len), 1)
end
...
function colon{T<:Union{Float16,Float32,Float64}}(start::T, step::T, stop::T)
    ...
    # Fall back to taking start and step at face value
...
function _convertSRL{T,R,S,U}(::Type{StepRangeLen{T,R,S}}, r::Range{U})
    ...
    step_n, step_d = rat(s)
    if U(start_n/start_d) == f && U(step_n/step_d) == s
        den = lcm(start_d, step_d)
        m = maxintfloat(T)
        if abs(f*den) <= m && abs(s*den) <= m
            start_n = round(Int, f*den)
            step_n = round(Int, s*den)
            if U(start_n/den) == f && U(step_n/den) == s
...
function +{T,R<:TwicePrecision}(r1::StepRangeLen{T,R}, r2::StepRangeLen{T,R})
    ...
    if r1.offset == r2.offset
        ...
        ref = TwicePrecision(ref_hi, ref_lo + (v1mid_lo + v2mid_lo))
    end
...
function linspace{T}(::Type{T}, start_n::Integer, stop_n::Integer, len::Integer, den::Integer)
    ...
    ref_hi, ref_lo = add2(start_itp.hi, stop_itp.hi)
    ref_hi, ref_lo = add2(ref_hi, ref_lo + (start_itp.lo + stop_itp.lo))
    # Compute step to 2x precision without risking overflow...
    rend = proddiv(T, (stop_n,), dent)
    rbeg = proddiv(T, (-start_n,), dent)
    step_hi_pre, step_lo = add2(rend.hi, rbeg.hi)
    step_hi_pre, step_lo = add2(step_hi_pre, step_lo + (rend.lo + rbeg.lo))
    # ...and then truncate enough low-bits of step_hi to ensure that
    # multiplication by 0:len-1 is exact
    nb = nbitslen(len, imin)
    step_hi = truncbits(step_hi_pre, nb)
    step_lo += step_hi_pre - step_hi
    StepRangeLen(TwicePrecision(ref_hi, ref_lo), TwicePrecision(step_hi, step_lo), Int(len), imin)
end
